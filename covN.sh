...
then
    ...
    bedops --everything {${name_vcf2},${name_vcf1}}_covN_{deletions,snvs}.bed | awk 'BEGIN{OFS="\t"}{print($1,$2,$3)}' > ${name_out}_covN.bed
    java -Xms512m -Xmx6G -jar $GATKJAR -T UnifiedGenotyper -R $GENOME -I $bam1 -o "$name_out.vcf" --intervals ${name_out}_covN.bed --output_mode EMIT_ALL_SITES > "$name_out.log" 2>&1
