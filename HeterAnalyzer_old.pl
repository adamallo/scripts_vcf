...
sub filter
{
    ...
    if (!-f $thisAname)
    {
        ...
        mysystem("$filtering_command -i $Aexecondname -o $thisAname") == 0 or die "Error filtering $Aexecondname to generate $thisAname\n. This is probably due to lack of memory for the java virtual machine\n" ; ## I don't think speed would be an issue, thus I'll call the filtering script every time (instead of writting a package and/or functions here)
...
#    print("DEBUG: $Aexecondname generated $thisAname using the filter $condition\n");
    ...
    if (!-f $thisBname)
    {
        ...
        ## Filter the right vcf file generated in the outside loop		
        mysystem("$filtering_command -i $Bexecondname -o $thisBname") == 0 or die "Error filtering $Bexecondname to generate $thisBname\n. This is probably due to lack of memory for the java virtual machine\n" ; ## I don't think speed would be an issue, thus I'll call the filtering script every time (instead of writting a package and/or functions here)
...
sub filterNAB
{
    ...
    if (!-f $thisNABname)
	{
		...
		mysystem("$filtering_command -i NAB.vcf -o $thisNABname") == 0 or die "Error filtering NAB.vcf to generate $thisNABname\n. This is probably due to lack of memory for the java virtual machine\n";
	}
	else
	{
